...
| 模块 | 内容 | 状态 |
|------|------|------|
| 🏗️ **架构重构** | **PlayerActor职责分离，引入DDD架构** | 🔥 **进行中** |
| └── PlayerModel | 纯数据模型，分离业务逻辑 | 🚧 实施中 |
| └── PlayerDomain | 领域服务，封装业务逻辑 | 🚧 待实现 |
| └── Command模式 | 解耦操作和执行 | 🚧 待实现 |
| └── 协调器Actor | 轻量级PlayerActor | 🚧 待实现 |
| 🔮 子项目系统 | 序列分支选择、解锁条件 | ⏸️ 暂停 |
| 🎯 成就系统 | 修炼里程碑、特殊奖励 | ⏸️ 暂停 |
| 💬 物品使用系统 | 消耗品使用、临时增益 | ⏸️ 暂停 |
